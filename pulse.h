...
// 速度最大值由驱动器和电机决定，有些最大是1800，有些可以达到4000
 uint32 V_max  = 10;   //(25/8*PI);         // 速度 单位为rad/sec
// 加速度和减速度选取一般根据实际需要，值越大速度变化越快，加减速阶段比较抖动
// 所以加速度和减速度值一般是在实际应用中多尝试出来的结果
 uint32 accel = 157;//(50*PI);         // 加速度 单位为rad/sec^2
 uint32 decel = 157;//(50*PI);         // 减速度 单位为rad/sec^2

#define  SYS_FREQ           ((double)16000000)

#define  TIMER_FREQ   		(SYS_FREQ/4/40) //10分频,4分频 16M/4*(1/40)
...
#define T_FREQ_148          ((float)(TIMER_FREQ*0.676)) 		// 0.676为误差修正值
